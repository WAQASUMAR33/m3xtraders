...
const FilterableTable = ({ data, fetchData }) => {
  ...
  const handleAddNewItem = async () => {
    ...
    if(newItem.id == null){
      ...
      try {
        const response = await fetch(`/api/account/${newItem.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="bg-white shadow rounded-lg p-4 relative">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800">Bank Accounts List</h2>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bank Title</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredData.map((item, index) => (
                <tr key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.bankTitle}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.accountNo}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.accountTitle}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-4 w-[700px] rounded shadow-lg">
            <h2 className="text-xl mb-4">{newItem.id ? 'Edit Item' : 'Add New Item'}</h2>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700">Bank Title</label>
              <input
                type="text"
                value={newItem.bankTitle}
                onChange={(e) => setNewItem({ ...newItem, bankTitle: e.target.value })}
            ...
            <div className="mb-4">
              ...
              <input
                type="text"
                value={newItem.accountNo}
                onChange={(e) => setNewItem({ ...newItem, accountNo: e.target.value })}
                className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div className="mb-4">
              ...
              <input
                type="text"
                value={newItem.accountTitle}
                onChange={(e) => setNewItem({ ...newItem, accountTitle: e.target.value })}
                className="mt-1 p-2 border border-gray-300 rounded w-full focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
